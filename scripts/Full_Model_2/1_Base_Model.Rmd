...
# Movement Parameters
M_ab = 0.8 # Adult movement A -> B 
M_ba = 0.1 # Adult movement B -> A
M_aa = 1 - M_ab
M_bb = 1 - M_ba
D_ab = 0.7 # Larval dispersal A -> B
...
# Prepare results for export and graphing
results_evals = results %>%
  mutate(accept = WTP_A-WTA_B >= 0)
  #filter(WTP_A > 0) %>% 
  #filter(WTA_B > 0) %>%
  #mutate(accept = WTP_A-WTA_B) %>% 
  #filter(accept > 0)

write.csv(results_evals,here("data","inputs","e_vals.csv"), row.names=FALSE)
...
results_filter = results %>% 
  filter(WTP_A > 0) %>% 
  filter(WTA_B > 0) %>%
  mutate(accept = WTP_A-WTA_B) # Positive values are accepted
