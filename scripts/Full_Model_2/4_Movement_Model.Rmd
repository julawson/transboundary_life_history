...
# Movement Parameters
M_ab = 0.3 # Adult movement A -> B 
M_ba = 0.5 # Adult movement B -> A
M_aa = 1 - M_ab
M_bb = 1 - M_ba
D_ab = 0.4 # Larval dispersal A -> B
D_ba = 0.5 # Larval dispersal B -> A
D_aa = 1 - D_ab
D_bb = 1 - D_ba
...
#max_accept_df <- data.frame(M_ab = numeric(), M_ba = numeric(), D_ab = numeric(), D_ba = numeric(), max_accept = numeric())
...
for(i in seq_along(x_a_grid)) {
  for(j in seq_along(x_b_grid)) {
    ...
    # Adding the current M_ab and D_ab to the results data frame.
        results <- results %>%
          add_row(
            x_a = x_a_grid[i],
            x_b = x_b_grid[j],
...
# Merging calculated values into results dataframe for calculating accept
    df_inf = df_all %>% filter(time == 1) %>% select(-time)
    
    results <- results

    # Calculating and appending max_accept value
    #max_accept <- results %>%
      #filter(WTP_A > 0) %>% filter(WTA_B > 0) %>%
      #mutate(accept = WTP_A-WTA_B) %>%
      #mutate(accept = ifelse(e_a_star == 0, NA, WTP_A - WTA_B)) %>%
      #slice_max(accept)
    
    #max_accept_df <- rbind(max_accept_df, data.frame(M_ab = M_ab, M_ba = M_ba, D_ab = D_ab, D_ba = D_ba, max_accept = max_accept))
...
#source_b_max_accept_df = max_accept_df
...
write.csv(results_filter, here("data","outputs","results.csv"), row.names=FALSE)
...
results_filter <- results_filter %>% 
  filter(x_a >= 10 & x_a <= 12) %>% 
  filter(x_b >= 10 & x_b <= 12)

for_graph <- results_filter %>%
  ggplot(aes(x = D_ab, y = accept, color=factor(M_ab))) + 
  geom_line() +
  theme_bw() 
