...
### Adding 'adaptive escapement' scenario into the Toy Model time series.
...
tpv_adaptive = results_adaptive %>%
  ...
  mutate(joint_payoff=sum(patch_a+patch_b)) %>%
  add_column(scenario = "adaptive")

tpv_compare = bind_rows(tpv_so, tpv_non, tpv_adaptive)
print(tpv_compare)

# Create Dataframe of e_star, e_hat, and e_adaptive

vals_so = results_so %>% 
  distinct(e_star_a,e_star_b) %>% 
  add_column(scenario = "sole owner") %>% 
  rename("patch_a"="e_star_a",
         "patch_b"="e_star_b")

vals_non = results_non %>% 
  distinct(e_hat_a,e_hat_b) %>% 
  add_column(scenario = "non-coop") %>% 
  rename("patch_a"="e_hat_a",
         "patch_b"="e_hat_b")

vals_compare = bind_rows(vals_so, vals_non)

#write.csv(vals_compare, here("data","vals_compare.csv"))
