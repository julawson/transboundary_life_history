...
ggplot(all_results, aes(x = e_a, y = a_1, fill=as.factor(M_ba), color = as.factor(D_ba))) +
  geom_line() +
...
#Plot for Patch A if D_ba ranges from 0 (no dispersal) to 1 (all larvae move from patch B to patch A)
ggplot(all_results, aes(x = e_b, y = b_1, fill=as.factor(M_ba), color = as.factor(D_ba))) +
  geom_line() +
  geom_abline(slope = 1, intercept = 0, linetype = "dashed", color = "red") +
